...
namespace RootMotion {
	...
	public class CameraController : MonoBehaviour 
	{
		...
		public Vector3 angleOffset = new Vector3(-90, 90, 0); // The offset from target relative to camera rotation
		...
		public void UpdateTransform(float deltaTime) {
			...
			// Rotation
			rotation = Quaternion.AngleAxis(x, transform.right) * Quaternion.AngleAxis(y, transform.right);
//			rotation = Quaternion.AngleAxis(x, Vector3.up) * Quaternion.AngleAxis(y, Vector3.right);
			...
			// Position
			position = smoothPosition + rotation * (offset - transform.forward * distance);
